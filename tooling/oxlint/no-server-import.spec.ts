...
const makeImportSpecifier = (importKind?: "value" | "type") => ({
  type: "ImportSpecifier" as const,
  imported: { type: "Identifier", name: "Context" },
  local: { type: "Identifier", name: "Context" },
  importKind,
});

const makeImportDecl = (
  source: string,
  importKind?: "value" | "type",
  specifiers: ReturnType<typeof makeImportSpecifier>[] = [],
) => ({
...
const makeRequireCall = (source: string) => ({
  type: "CallExpression" as const,
  callee: { type: "Identifier", name: "require" },
  arguments: [makeStringLiteral(source)],
  optional: false,
});

const makeImportEquals = (source: string, importKind: "value" | "type") => ({
  type: "TSImportEqualsDeclaration" as const,
  id: { type: "Identifier", name: "ServerModule" },
  importKind,
  moduleReference: {
    type: "TSExternalModuleReference" as const,
    expression: makeStringLiteral(source),
  },
});
...
const makeExportSpecifier = (exportKind?: "value" | "type") => ({
  type: "ExportSpecifier" as const,
  local: { type: "Identifier", name: "Context" },
  exported: { type: "Identifier", name: "Context" },
  exportKind,
});

const makeExportNamedWithSpecifiers = (
  source: string,
  specifiers: ReturnType<typeof makeExportSpecifier>[],
) => ({
  ...makeExportNamed(source, "value"),
  specifiers,
});
...
describe("no-server-import", () => {
  describe("ImportDeclaration", () => {
    ...
    it("reports every Node builtin through the node: namespace", () => {
      const ctx = createMockContext([{ forbidden: ["node:"] }]);
      const visitors = noServerImport.create(ctx as any);
      visitors.ImportDeclaration!(makeImportDecl("node:console") as any);
      expect(ctx.report).toHaveBeenCalledOnce();
    });
    ...
    it("allows imports whose bindings are all inline type-only", () => {
      const ctx = createMockContext(DEFAULT_OPTIONS);
      const visitors = noServerImport.create(ctx as any);
      visitors.ImportDeclaration!(
        makeImportDecl("hono", "value", [makeImportSpecifier("type")]) as any,
      );
      expect(ctx.report).not.toHaveBeenCalled();
    });
    ...
    it("rejects mixed inline type and value imports", () => {
      const ctx = createMockContext(DEFAULT_OPTIONS);
      const visitors = noServerImport.create(ctx as any);
      visitors.ImportDeclaration!(
        makeImportDecl("hono", "value", [
          makeImportSpecifier("type"),
          makeImportSpecifier("value"),
        ]) as any,
      );
      expect(ctx.report).toHaveBeenCalledOnce();
    });
  ...
  describe("CommonJS require", () => {
    it("rejects literal requires of forbidden packages", () => {
      const ctx = createMockContext(DEFAULT_OPTIONS);
      const visitors = noServerImport.create(ctx as any);
      visitors.CallExpression!(makeRequireCall("hono") as any);
      expect(ctx.report).toHaveBeenCalledOnce();
    });

    it("rejects TypeScript import-equals requires of forbidden packages", () => {
      const ctx = createMockContext(DEFAULT_OPTIONS);
      const visitors = noServerImport.create(ctx as any);
      visitors.TSImportEqualsDeclaration!(
        makeImportEquals("hono", "value") as any,
      );
      expect(ctx.report).toHaveBeenCalledOnce();
    });
  });
  ...
  describe("ExportNamedDeclaration", () => {
    ...
    it("allows re-exports whose bindings are all inline type-only", () => {
      const ctx = createMockContext(DEFAULT_OPTIONS);
      const visitors = noServerImport.create(ctx as any);
      visitors.ExportNamedDeclaration!(
        makeExportNamedWithSpecifiers("hono", [
          makeExportSpecifier("type"),
        ]) as any,
      );
      expect(ctx.report).not.toHaveBeenCalled();
    });
