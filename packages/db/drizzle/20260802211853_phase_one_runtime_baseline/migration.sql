...
CREATE TYPE "OperationFailureAuthorizationDecision" AS ENUM('api_key_scope_denied', 'rebac_denied', 'write_mode_denied');--> statement-breakpoint
CREATE TYPE "OperationFailureBlocker" AS ENUM('branch_translation_write_failed', 'branch_write_context_unavailable', 'candidate_channel_capability_unavailable', 'candidate_channel_execution_failed', 'language_analysis_duplicate_implementation', 'language_analysis_installation_scope_mismatch', 'language_analysis_invalid_attestation', 'language_analysis_invalid_configuration', 'language_analysis_invalid_response', 'language_analysis_missing_implementation', 'language_analysis_missing_selection', 'language_analysis_policy_changed', 'language_analysis_service_type_mismatch', 'language_analysis_timeout', 'language_analysis_unavailable', 'language_analysis_unsupported_language', 'recall_derivation_blocked', 'recall_derivation_failed', 'recall_derivation_pending', 'recall_derivation_stale', 'reviewable_change_write_failed');--> statement-breakpoint
CREATE TYPE "OperationFailureCapability" AS ENUM('CANDIDATE_RECALL', 'LANGUAGE_ANALYSIS', 'RECALL_DERIVATION', 'VECTOR_STORAGE', 'TEXT_VECTORIZER');--> statement-breakpoint
CREATE TYPE "OperationFailureCode" AS ENUM('CAT_OPERATION_CANCELED', 'CAT_OPERATION_DEPENDENCY_UNAVAILABLE', 'CAT_OPERATION_EXECUTION_DENIED', 'CAT_OPERATION_INVALID_INPUT', 'CAT_OPERATION_MISSING_CAPABILITY', 'CAT_OPERATION_RESOURCE_NOT_FOUND', 'CAT_OPERATION_FAILED', 'CAT_OPERATION_PERMISSION_DENIED', 'CAT_OPERATION_RELATIONSHIP_DENIED', 'CAT_OPERATION_REVIEW_CHANGE_BLOCKED');--> statement-breakpoint
CREATE TYPE "OperationFailureRedactionBoundary" AS ENUM('PUBLIC', 'INTERNAL');--> statement-breakpoint
CREATE TYPE "OperationFailureSeverity" AS ENUM('INFO', 'WARNING', 'ERROR');--> statement-breakpoint
CREATE TYPE "PermissionAction" AS ENUM('check', 'grant', 'revoke');--> statement-breakpoint
CREATE TYPE "PluginServiceType" AS ENUM('AUTH_FACTOR', 'STORAGE_PROVIDER', 'FILE_IMPORTER', 'FILE_EXPORTER', 'TRANSLATION_ADVISOR', 'TEXT_VECTORIZER', 'VECTOR_STORAGE', 'QA_CHECKER', 'TOKENIZER', 'LLM_PROVIDER', 'RERANK_PROVIDER', 'AGENT_TOOL_PROVIDER', 'AGENT_CONTEXT_PROVIDER', 'LANGUAGE_ANALYZER', 'EMAIL_PROVIDER');--> statement-breakpoint
...
CREATE TYPE "RecallDerivationStatus" AS ENUM('PENDING', 'RUNNING', 'FRESH', 'BLOCKED', 'FAILED');--> statement-breakpoint
CREATE TYPE "RecallDerivationTargetKind" AS ENUM('MEMORY_ITEM', 'TERM_CONCEPT');--> statement-breakpoint
...
CREATE TYPE "TaskActorType" AS ENUM('USER', 'SYSTEM');--> statement-breakpoint
CREATE TYPE "TaskKind" AS ENUM('BATCH_AUTO_TRANSLATION', 'RECALL_DERIVATION');--> statement-breakpoint
CREATE TYPE "TaskScopeType" AS ENUM('PROJECT', 'USER', 'INSTANCE');--> statement-breakpoint
CREATE TYPE "TaskStatus" AS ENUM('PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'BLOCKED', 'CANCEL_REQUESTED', 'CANCELED');--> statement-breakpoint
...
CREATE TYPE "WorkflowTaskDispatchStatus" AS ENUM('REQUESTED', 'CLAIMED', 'RUNNING', 'CANCELLING', 'SETTLED');--> statement-breakpoint
...
CREATE TABLE "Blob" (
	"id" serial PRIMARY KEY,
	"key" text NOT NULL,
	"storage_provider" jsonb NOT NULL,
	"reference_count" integer DEFAULT 1 NOT NULL,
	"hash" bytea UNIQUE,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "Blob_storage_provider_key_unique" UNIQUE("storage_provider","key"),
...
CREATE TABLE "Chunk" (
	...
	"vectorizer" jsonb NOT NULL,
	"vector_storage" jsonb NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE "Memory" (
	...
	"name" text NOT NULL,
	"description" text,
	"scope" "MemoryScope" DEFAULT 'PROJECT'::"MemoryScope" NOT NULL,
	"creator_id" uuid NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "MemoryRecallVariant" (
	"id" serial PRIMARY KEY,
	"derivation_state_id" integer NOT NULL,
	...
	"meta" jsonb,
	"canonical_input_version" text NOT NULL,
	"recall_derivation_version" text NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE "OperationFailure" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"code" "OperationFailureCode" NOT NULL,
	"message" text NOT NULL,
	"severity" "OperationFailureSeverity" NOT NULL,
	"retryable" boolean NOT NULL,
	"blocker" "OperationFailureBlocker",
	"capability" "OperationFailureCapability",
	"authorization_decision" "OperationFailureAuthorizationDecision",
	"affected_resources" jsonb NOT NULL,
	"remediation_hint" text,
	"redaction_boundary" "OperationFailureRedactionBoundary" NOT NULL,
	"task_id" uuid,
	"trace_id" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "RecallDerivationState" (
	"id" serial PRIMARY KEY,
	"target_kind" "RecallDerivationTargetKind" NOT NULL,
	"target_id" text NOT NULL,
	"language_id" text NOT NULL,
	"status" "RecallDerivationStatus" DEFAULT 'PENDING'::"RecallDerivationStatus" NOT NULL,
	"demand_revision" integer DEFAULT 1 NOT NULL,
	"task_projection_revision" integer DEFAULT 1 NOT NULL,
	...
	"current_canonical_input_version" text,
	"current_derivation_version" text,
	"last_attempt_at" timestamp with time zone,
	"published_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "RecallDerivationState_running_lease_check" CHECK (("status" = 'RUNNING' AND "execution_epoch" > 0 AND "lease_owner_id" IS NOT NULL AND "lease_token" IS NOT NULL AND "lease_expires_at" IS NOT NULL) OR ("status" <> 'RUNNING' AND "lease_owner_id" IS NULL AND "lease_token" IS NULL AND "lease_expires_at" IS NULL)),
	CONSTRAINT "RecallDerivationState_fresh_version_check" CHECK ("status" <> 'FRESH' OR ("canonical_input_version" IS NOT NULL AND "required_derivation_version" IS NOT NULL AND "current_canonical_input_version" IS NOT NULL AND "current_derivation_version" IS NOT NULL AND "current_canonical_input_version" = "canonical_input_version" AND "current_derivation_version" = "required_derivation_version")),
	CONSTRAINT "RecallDerivationState_revision_check" CHECK ("demand_revision" > 0 AND "task_projection_revision" > 0 AND "execution_epoch" >= 0 AND "retry_count" >= 0)
);
--> statement-breakpoint
CREATE TABLE "RecallDerivationTaskDemand" (
	"id" serial PRIMARY KEY,
	"task_id" uuid NOT NULL,
	"derivation_state_id" integer,
	"target_kind" "RecallDerivationTargetKind" NOT NULL,
	"target_id" text NOT NULL,
	"language_id" text NOT NULL,
	"demand_revision" integer NOT NULL,
	"observed_projection_revision" integer NOT NULL,
	"detached_at" timestamp with time zone,
	"superseded_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "RecallDerivationTaskDemand_task_id_target_kind_target_id_language_id_demand_revision_unique" UNIQUE("task_id","target_kind","target_id","language_id","demand_revision"),
	CONSTRAINT "RecallDerivationTaskDemand_revision_check" CHECK ("demand_revision" > 0 AND "observed_projection_revision" > 0)
);
--> statement-breakpoint
CREATE TABLE "Role" (
	"id" serial PRIMARY KEY,
...
CREATE TABLE "Task" (
	...
	"scope_type" "TaskScopeType" NOT NULL,
	"scope_id" uuid,
	"actor_type" "TaskActorType" NOT NULL,
	"actor_id" uuid,
	...
	"current_failure_id" uuid,
	"retry_of_task_id" uuid,
	"started_at" timestamp with time zone,
	"finished_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "Task_scope_contract_check" CHECK ((("scope_type" IN ('PROJECT', 'USER') AND "scope_id" IS NOT NULL) OR ("scope_type" = 'INSTANCE' AND "scope_id" IS NULL))),
	CONSTRAINT "Task_actor_contract_check" CHECK ((("actor_type" = 'USER' AND "actor_id" IS NOT NULL) OR ("actor_type" = 'SYSTEM' AND "actor_id" IS NULL))),
	CONSTRAINT "Task_progress_contract_check" CHECK (("progress_current" IS NULL OR "progress_current" >= 0) AND ("progress_total" IS NULL OR "progress_total" > 0) AND ("progress_current" IS NULL OR "progress_total" IS NULL OR "progress_current" <= "progress_total"))
);
--> statement-breakpoint
CREATE TABLE "TaskTransitionRequest" (
	"task_id" uuid,
	"request_id" uuid,
	"intent_fingerprint" text NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "TaskTransitionRequest_pkey" PRIMARY KEY("task_id","request_id")
);
--> statement-breakpoint
CREATE TABLE "Term" (
	"id" serial PRIMARY KEY,
...
--> statement-breakpoint
CREATE TABLE "WorkflowTaskDispatch" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"task_id" uuid NOT NULL,
	"generation" integer NOT NULL,
	"run_id" uuid DEFAULT gen_random_uuid() NOT NULL UNIQUE,
	"status" "WorkflowTaskDispatchStatus" DEFAULT 'REQUESTED'::"WorkflowTaskDispatchStatus" NOT NULL,
	"owner_id" uuid,
	"owner_epoch" integer DEFAULT 0 NOT NULL,
	"owner_lease_expires_at" timestamp with time zone,
	"attempt_count" integer DEFAULT 0 NOT NULL,
	"agent_session_id" integer,
	"last_projected_event_sequence" integer DEFAULT 0 NOT NULL,
	"settled_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "WorkflowTaskDispatch_task_id_generation_unique" UNIQUE("task_id","generation")
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "AgentRun_deduplication_key_index" ON "AgentRun" ("deduplication_key");--> statement-breakpoint
...
CREATE INDEX "LanguageAnalysisObservation_selection_key_index" ON "LanguageAnalysisObservation" ("selection_key");--> statement-breakpoint
...
CREATE INDEX "MemoryRecallVariant_derivation_state_id_canonical_input_version_recall_derivation_version_index" ON "MemoryRecallVariant" ("derivation_state_id","canonical_input_version","recall_derivation_version");--> statement-breakpoint
...
CREATE UNIQUE INDEX "RecallDerivationState_target_kind_target_id_language_id_index" ON "RecallDerivationState" ("target_kind","target_id","language_id");--> statement-breakpoint
CREATE UNIQUE INDEX "RecallDerivationState_lease_token_index" ON "RecallDerivationState" ("lease_token");--> statement-breakpoint
CREATE INDEX "RecallDerivationState_status_next_attempt_at_index" ON "RecallDerivationState" ("status","next_attempt_at");--> statement-breakpoint
CREATE INDEX "RecallDerivationState_lease_expires_at_index" ON "RecallDerivationState" ("lease_expires_at");--> statement-breakpoint
CREATE INDEX "RecallDerivationTaskDemand_derivation_state_id_detached_at_index" ON "RecallDerivationTaskDemand" ("derivation_state_id","detached_at");--> statement-breakpoint
...
CREATE INDEX "Task_scope_type_scope_id_index" ON "Task" ("scope_type","scope_id");--> statement-breakpoint
CREATE INDEX "Task_status_index" ON "Task" ("status");--> statement-breakpoint
CREATE UNIQUE INDEX "Task_retry_of_task_id_unique" ON "Task" ("retry_of_task_id");--> statement-breakpoint
...
CREATE INDEX "TermRecallVariant_derivation_state_id_canonical_input_version_recall_derivation_version_index" ON "TermRecallVariant" ("derivation_state_id","canonical_input_version","recall_derivation_version");--> statement-breakpoint
...
CREATE INDEX "WorkflowTaskDispatch_status_owner_lease_expires_at_index" ON "WorkflowTaskDispatch" ("status","owner_lease_expires_at");--> statement-breakpoint
CREATE INDEX "WorkflowTaskDispatch_task_id_generation_index" ON "WorkflowTaskDispatch" ("task_id","generation");--> statement-breakpoint
...
ALTER TABLE "MemoryItem" ADD CONSTRAINT "MemoryItem_memory_id_Memory_id_fkey" FOREIGN KEY ("memory_id") REFERENCES "Memory"("id") ON DELETE RESTRICT ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "MemoryRecallVariant" ADD CONSTRAINT "MemoryRecallVariant_x8fYpbOX6tOL_fkey" FOREIGN KEY ("derivation_state_id") REFERENCES "RecallDerivationState"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "PersonalMemoryBinding" ADD CONSTRAINT "PersonalMemoryBinding_user_id_User_id_fkey" FOREIGN KEY ("user_id") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "PluginConfigInstance" ADD CONSTRAINT "PluginConfigInstance_creator_id_User_id_fkey" FOREIGN KEY ("creator_id") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "RecallDerivationState" ADD CONSTRAINT "RecallDerivationState_language_id_Language_id_fkey" FOREIGN KEY ("language_id") REFERENCES "Language"("id") ON DELETE RESTRICT ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "RecallDerivationTaskDemand" ADD CONSTRAINT "RecallDerivationTaskDemand_task_id_Task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "Task"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "RecallDerivationTaskDemand" ADD CONSTRAINT "RecallDerivationTaskDemand_YzwgUZDa1Aya_fkey" FOREIGN KEY ("derivation_state_id") REFERENCES "RecallDerivationState"("id") ON DELETE SET NULL ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "Task" ADD CONSTRAINT "Task_actor_id_User_id_fkey" FOREIGN KEY ("actor_id") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "Task" ADD CONSTRAINT "Task_current_failure_id_OperationFailure_id_fkey" FOREIGN KEY ("current_failure_id") REFERENCES "OperationFailure"("id") ON DELETE SET NULL ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "Task" ADD CONSTRAINT "Task_retry_of_task_id_Task_id_fk" FOREIGN KEY ("retry_of_task_id") REFERENCES "Task"("id") ON DELETE SET NULL ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "TaskTransitionRequest" ADD CONSTRAINT "TaskTransitionRequest_task_id_Task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "Task"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "TermRecallVariant" ADD CONSTRAINT "TermRecallVariant_gWl17KSRCDjQ_fkey" FOREIGN KEY ("derivation_state_id") REFERENCES "RecallDerivationState"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
...
ALTER TABLE "VectorizedString" ADD CONSTRAINT "VectorizedString_chunk_set_id_ChunkSet_id_fkey" FOREIGN KEY ("chunk_set_id") REFERENCES "ChunkSet"("id") ON DELETE RESTRICT ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "WorkflowTaskDispatch" ADD CONSTRAINT "WorkflowTaskDispatch_task_id_Task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "Task"("id") ON DELETE CASCADE ON UPDATE CASCADE;--> statement-breakpoint
ALTER TABLE "WorkflowTaskDispatch" ADD CONSTRAINT "WorkflowTaskDispatch_agent_session_id_AgentSession_id_fkey" FOREIGN KEY ("agent_session_id") REFERENCES "AgentSession"("id") ON DELETE SET NULL ON UPDATE CASCADE;