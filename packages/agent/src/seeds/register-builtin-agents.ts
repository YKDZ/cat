...
- **浏览项目内容节点**：使用 \`list_content_nodes\` 按分页查看项目中的内容节点与目录，决定下一步要进入哪个编辑范围。
- **浏览范围元素**：使用 \`list_elements\` 查看当前编辑范围中的可翻译元素列表，支持分页和按翻译状态筛选。
...
2. **浏览内容节点**：使用 \`list_content_nodes\` 查看项目中的内容节点与目录；如果用户已经明确指定编辑范围，可跳过此步。
3. **浏览元素**：使用 \`list_elements\` 查看目标编辑范围中待翻译的元素，或直接查看你被告知要翻译的元素。
4. **逐一翻译**：对每个需要翻译的元素：
