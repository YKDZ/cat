...
export type TypedEventHandler<TEvent> = (event: TEvent) => MaybePromise<void>;
...
> = {
  ...
  subscribeAll: (handler: TypedEventHandler<TEvent>) => () => void;
...
export class InProcessEventBus<
  TType extends string,
  TEvent extends { type: TType },
> implements TypedEventBus<TType, TEvent> {
  private readonly handlersByType = new Map<
    TType,
    Set<TypedEventHandler<TEvent>>
  >();

  private readonly allHandlers = new Set<TypedEventHandler<TEvent>>();
