...
const seedFixture = async () => {
    ...
    createElements,
    {
      data: [
        ...
        },
        {
          ...
          stringId: requireFixtureValue(
            sourceIdByValue.get("Click Checkout to continue"),
          ),
          localOrder: 2,
        },
        {
          ...
          stringId: requireFixtureValue(sourceIdByValue.get("Checkout")),
          localOrder: 3,
        },
      ],
    },
  ...
  return {
    ...
    elementIds: {
      ...
      checkoutLong: requireFixtureValue(elementIds[4]),
      checkoutSeed: requireFixtureValue(elementIds[5]),
    },
  };
};
...
describe("editor scope elements", () => {
  ...
  test("reuse-first ordering is shared by list, first element, next untranslated, and page index", async () => {
    ...
    expect(requireFixtureValue(firstPage[0])?.id).toBe(
      fixture.elementIds.checkoutSeed,
    );
    expect(first?.id).toBe(fixture.elementIds.checkoutSeed);
    expect(requireFixtureValue(firstPage[0])?.priority?.reasonCodes).toContain(
      "REUSE_SEED",
    );
    expect(seedPage).toBe(0);
    expect(requireFixtureValue(secondPage[0])?.id).toBe(nextAfterSeed?.id);
  });
