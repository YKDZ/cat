...
describe("Trust-by-default & Isolation Exception", () => {
  it("默认: grant editor 后 Subject 自动获得 direct_editor（联写合约）", async () => {
    ...
    // Here we verify the trust mode is "trust" when both are present.
    const engine = makeEngine(["editor", "direct_editor"]);
    await expect(
      determineTrustMode(engine, makeAuthCtx(), PROJECT_ID),
    ).resolves.toBe("trust");
  });

  it("默认: editor + direct_editor 的 Subject 直接编辑 main (trust mode)", async () => {
    const engine = makeEngine(["editor", "direct_editor"]);
    const mode = await determineTrustMode(engine, makeAuthCtx(), PROJECT_ID);
  ...
  it("例外: revoke isolation_forced 后恢复 Trust 模式", async () => {
    // Before revoke: isolation_forced present → isolation
    const before = makeEngine(["editor", "direct_editor", "isolation_forced"]);
    expect(await determineTrustMode(before, makeAuthCtx(), PROJECT_ID)).toBe(
      "isolation",
    );

    // After revoke: isolation_forced removed → trust
    const after = makeEngine(["editor", "direct_editor"]);
    expect(await determineTrustMode(after, makeAuthCtx(), PROJECT_ID)).toBe(
      "trust",
    );
  });
  ...
  it("前提: PR 功能未启用时，isolation 仍可通过权限判定（功能检查在 router 层）", async () => {
    // determineTrustMode only cares about permission tuples, not feature flags.
