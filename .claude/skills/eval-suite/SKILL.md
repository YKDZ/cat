...
每个 suite 必须在根目录提供 README.md，至少包含以下章节：

| 章节         | 要求                                                                 |
| ------------ | -------------------------------------------------------------------- |
| **起因**     | 为什么创建这个测试套件？解决了什么具体问题？列出对应的问题类别和组件 |
| **测试集**   | 列出所有测试集：名称、类型、用例数、验证目标、关键阈值               |
| **种子数据** | 种子数据的来源、规模和覆盖范围                                       |
| **评分指标** | 每个指标的含义和对应的测试集                                         |
| **使用方法** | 运行命令和可选参数（--scenario、--clear-cache、--output 等）         |
| **备注**     | 已知限制、外部依赖、数据说明                                         |
