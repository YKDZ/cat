...
import { clearTokenCache, getAuthEnv } from "../shared/github-app-auth.js";
import {
  listPRComments,
  listPRs,
...
const WAIT_COMPLETION_MS = 300_000; // 5 min for agent to finish
const WAIT_RETRIGGER_MS = 180_000; // 3 min for re-trigger cycle
...
describe.skipIf(!E2E_ENABLED)("Auto-Dev full lifecycle E2E", () => {
  ...
  beforeAll(async () => {
    ...
    // Set short poll interval for E2E tests
    process.env.AUTO_DEV_POLL_INTERVAL ??= "5";
    ...
    execSync(`git clone https://github.com/${REPO}.git "${workspaceRoot}" --depth=1`, {
      stdio: "pipe",
    });
    ...
    issueNumber = createIssue(
      ...
      ["auto-dev:ready", "agent:claude-code", "effort:max"],
    );
  ...
  it("coordinator picks up issue and creates a PR with BOT_MARKER comment", async () => {
    ...
    // Verify BOT_MARKER appears in PR comments
    const prWithMarker = await poll(
      () => {
        const comments = listPRComments(REPO, prNumber);
        return comments.find((c) => c.body.includes(BOT_MARKER));
      },
      30_000,
    );
  ...
  it("agent makes at least one commit to the PR branch", async () => {
    // Wait for a commit on the PR branch beyond the initial empty-branch setup
    const branchHasCommit = await poll(
      () => {
        try {
          const log = gh([
            "api",
            `repos/${REPO}/commits?sha=${prBranch}&per_page=5`,
            "--jq",
            ".[].commit.message",
          ]);
          // branch should have at least one commit authored by the bot
          return log.includes("chore:") || log.includes("e2e-marker") || log.split("\n").length >= 2;
        } catch {
          return false;
        }
      },
  ...
  it("coordinator posts a completion comment on the PR", async () => {
  ...
  it("@autodev re-trigger in PR comment causes agent to resume and commit again", async () => {
    // Record current commit count before re-trigger
    const commitsBefore = gh([
      "api",
      `repos/${REPO}/commits?sha=${prBranch}&per_page=20`,
      "--jq",
      "length",
    ]).trim();
    const countBefore = parseInt(commitsBefore, 10) || 0;

    // Post a @autodev re-trigger comment
    postComment(
      prNumber,
      `${AUTODEV_TRIGGER} Please also add a line \`<!-- e2e-retrigger -->\` to the marker file.`,
    );
    console.log("[e2e] Posted @autodev re-trigger comment");

    // Wait for a new commit to appear (count increases)
    const newCommit = await poll(
      () => {
        const countStr = gh([
          "api",
          `repos/${REPO}/commits?sha=${prBranch}&per_page=20`,
