...
export const AutoDevConfigSchema = z.object({
  ...
  pollIntervalSec: z.number().int().min(10).max(3600).default(30),
  maxDecisionPerRun: z.number().int().min(1).max(100).default(20),
  maxImplCycles: z.number().int().min(1).max(50).default(5),
});
