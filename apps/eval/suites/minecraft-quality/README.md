...
CAT 平台的翻译记忆召回与术语匹配在 Minecraft 数据集上暴露了 7 类系统性质量问题。为量化验证修复效果，设计此评测套件覆盖所有问题类别。
...
| 2   | 占位符模板匹配失效           | `1.20` → `1.21` 等版本号变体仅靠 pg_trgm 相似度，丢失结构性等价关系  | Template Structure Matcher (TMPL) |
| 3   | 短术语 trigram 假阳性        | 2-4 字符短术语的 word_similarity 阈值固定，对短字符串过于宽松        | Trigram Length Guard (TRGM_GUARD) |
| 4   | 自匹配噪音干扰               | 当前元素的自身记忆条目出现在召回结果中，稀释有效建议                 | Self-Exclusion Filter (SELF_EX)   |
| 5   | 相邻翻译上下文缺失           | LLM 翻译时缺少当前编辑会话中刚完成的相邻条目翻译                     | Session Translation Context       |
| 6   | 建议按到达时间排序           | LLM 建议因生成延迟晚于 advisor 到达，导致最佳建议沉底                | Quality Sorter (QUAL_SORT)        |
| 7   | 短术语 ILIKE 回退缺失        | 短术语通过 trigram 匹配后缺少完整词边界校验                          | TRGM_GUARD (ILIKE check)          |
...
| keyword-recall         | memory-recall   | 1      | 短文本的 analyzer token/lemma 与新鲜 Recall Variant 重叠          | channel-coverage ≥ 1.0    |
...
| channel-coverage       | 预期召回通道实际参与匹配的比例            | keyword-recall / template-matching                       |
...
- keyword-recall 使用 Language Analyzer 产生的内容 token/lemma，与新鲜 Recall Variant 做作用域内重叠；它不使用 pg_trgm。pg_trgm 属于独立的 fuzzy/trigram 通道。
