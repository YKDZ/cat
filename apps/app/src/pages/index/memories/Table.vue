...
type MemorySortColumnId = "createdAt" | "name" | "updatedAt";
type MemoryColumnId = "description" | MemorySortColumnId;
...
const sorting = ref<readonly DataTableSort<MemorySortColumnId>[]>([]);
...
const columns: readonly DataTableColumn<MemoryListItem, MemoryColumnId>[] = [
  {
    id: "name",
    header: t("名称"),
    render: (memory) => memory.name,
    sortable: true,
  },
  {
    id: "description",
...
const fetchMemories = async () => {
  ...
  const result = await requestOwnership.run(() => {
    ...
    if (sort !== undefined && searchArgument.length === 0) {
      return onRequestMemories(pageIndex.value, pageSize.value, null, sort);
    }
    return onRequestMemories(
      pageIndex.value,
      pageSize.value,
      ...searchArgument,
      ...(sort === undefined ? [] : [sort]),
...
const isSupportedSort = (
  sort: DataTableSort<MemoryColumnId>,
): sort is DataTableSort<MemorySortColumnId> => sort.id !== "description";

const updateSorting = (next: readonly DataTableSort<MemoryColumnId>[]) => {
