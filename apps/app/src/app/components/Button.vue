...
const dynBtnClasses = computed(() => {
  if (!props.large)
    return {
      ...
      "w-fit px-3.5 py-2.5 gap-1.5": !props.fullWidth && !props.noText,
      "w-10 h-10 gap-0": props.noText,
      "py-1.5": !props.noText,
      "text-highlight bg-base": !props.transparent && !props.disabled,
      "text-black bg-transparent":
        props.transparent && !props.disabled && !props.focused,
      "hover:bg-base-darker":
        !props.focused && !props.transparent && !props.disabled,
      "hover:bg-gray-200":
        !props.focused && props.transparent && !props.disabled,
      "bg-gray-200": props.focused && !props.disabled,
      "bg-gray-200 text-black cursor-not-allowed": props.disabled,
    ...
    return {
      "w-full": props.fullWidth && !props.noText,
      "w-fit px-4.5 py-3.5 gap-1.5": !props.fullWidth && !props.noText,
      "w-12 h-12 gap-0": props.noText,
...
<template>
  <button
    class="rounded-md flex select-none items-center"
    :class="dynBtnClasses"
  >
