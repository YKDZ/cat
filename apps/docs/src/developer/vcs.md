...
路由处理器通过中间件 `with-branch-context` 获取分支上下文，然后按优先级依次检查：隔离模式（分支存在）→ Direct 模式（提供了 `projectId`）→ 常规写入（无 VCS 上下文）。
...
- `apps/app-api/src/orpc/routers/document.ts` — 文档创建（Direct + Isolation）
- `apps/app-api/src/orpc/routers/translation.ts` — 译文创建（Direct + Isolation）
- `apps/app-api/src/orpc/routers/glossary.ts` — 术语创建 / 删除（Direct + Isolation）
- `apps/app-api/src/orpc/routers/comment.ts` — 评论创建（Direct + Isolation）
- `apps/app-api/src/orpc/routers/memory.ts` — 翻译记忆创建（Direct + Isolation）
- `apps/app-api/src/orpc/routers/project.ts` — 项目创建 / 更新 / 删除（Direct）
- `apps/app-api/src/orpc/routers/issue.ts` — Issue 创建（Direct）

### Graph Runtime VCS 集成

图执行引擎（`@cat/workflow`）也支持 VCS 上下文传播。在调用 `runGraph()` 或 `startGraph()` 时，传入 `vcsContext` 和 `vcsMiddleware` 选项，引擎会将它们注入到每个图节点的 `TypedNodeContext` 中。
...
图节点内使用 `executeWithVCS()` 辅助函数（来自 `@cat/workflow`）包裹写操作：
